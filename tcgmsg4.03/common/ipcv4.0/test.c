...
#define MAXLENG 256*1024
  ...
  for (len=1; len<MAXLENG/2; len*=2) {
    ...
    if (me == 0) {
      printf("global sums OK\n    TIME: %g used, %g overhead\n",
	  used / nloops, overhead / nloops);
      fflush(stdout);
    }
  }
    ...
    char **argv;
{
  ...
  while (1) {
    ...
    again:
      (void) printf("\n\
0=quit, 1=Ring, 2=Stress, 3=Hello, 4=RcvAny, 5=NxtVal, 6=Global, 7=Debug: ");
