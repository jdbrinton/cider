/* $Header: /home/harrison/c/tcgmsg/ipcv4.0/RCS/evlog.c,v 1.1 91/12/06 17:26:28 harrison Exp Locker: harrison $ */
...
#include <string.h>
    ...
    defined(AIX)     || defined(IPSC)   || defined(NEXT)
extern char *strdup();
#endif
...
#define  MAX_EV_LEN 1000   /* Assumed maximum size of single event record */
  static FILE *EVfile;     /* File where events will be dumped */
  static char *EVfilename = "events";   /* Default name of events file */
...
#define ERROR_RETURN if (!error) fputs("Event Log Error\n", stderr); \
		error = 1; return; \

#define DUMPBUF (void) fputs(buffer, EVfile); \
                (void) fflush(EVfile);\
                if(ferror(EVfile)) {ERROR_RETURN} \
...
#define RECORD(A)  A; nchars = strlen(bufpt); bufpt += nchars; left -= nchars
  ...
  if (ncall == 0) {
    ...
    if (!(EVfile = fopen(EVfilename, "w"))) {ERROR_RETURN}
  }
...
/*      break; */
    ...
    case EVKEY_FILENAME:
      EVfilename = strdup(va_arg(ap, char *));
      if (!(EVfile = freopen(EVfilename, "w", EVfile))) {ERROR_RETURN}
      break;
