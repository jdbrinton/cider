  ...
  ONEdevice *pDevice;
{
  ...
  for (eIndex = 1; eIndex < pDevice->numNodes; eIndex++) {
	...
	*(pNode->fPN) += dx * pNode->dUdN;
      ...
      if (pNode1->nodeType ISNOT CONTACT) {
	*(pNode->fPsiPsiiP1) -= rDx;
	if (pElem->elemType IS SEMICON) {
	  *(pNode->fNPsiiP1) += pEdge->dJnDpsiP1;
	  *(pNode->fNNiP1) += pEdge->dJnDnP1;
...
void 
ONE_commonTerms(pDevice, currentOnly, tranAnalysis, info)
  ONEdevice *pDevice;
  ...
  ONEtranInfo *info;
{
  ...
  ONEnode *pNode, *pNode1;
  int index, eIndex;
  double psi1, psi2, psi, nConc, pConc, nC, pC, nP1, pP1;
  double dPsiN, dPsiP;
  ...
  for (eIndex = 1; eIndex < pDevice->numNodes; eIndex++) {
	...
	if (pNode->nodeType ISNOT CONTACT) {
	  ...
	  if (pElem->elemType IS SEMICON) {
	    ...
	    if (pNode->baseType IS P_TYPE AND pConc <= 0.0) {
	      pConc = pNode->na;
	    } else if (pNode->baseType IS N_TYPE AND nConc <= 0.0) {
	      nConc = pNode->nd;
	    }
	  }
	} else {
  ...
  for (eIndex = 1; eIndex < pDevice->numNodes; eIndex++) {
    ...
    if (pElem->elemType IS SEMICON) {
      ...
      bernoulli(dPsiN, &bPsiN, &dbPsiN, &bMPsiN, &dbMPsiN, NOT currentOnly);
      bernoulli(dPsiP, &bPsiP, &dbPsiP, &bMPsiP, &dbMPsiP, NOT currentOnly);
