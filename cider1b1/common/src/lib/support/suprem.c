...
void
readAsciiData( fileName, impType, ppTable )
char *fileName;
int impType;
DOPtable **ppTable;
{
    ...
    ALLOC( profileData, double *, 2 );
    for( index = 0; index <= 1; index++ ) {
	ALLOC( profileData[ index ], double, 1 + numPoints );
    }
    ...
    ALLOC( tmpTable, DOPtable, 1 );
    if ( *ppTable IS NIL(DOPtable) ) {
      /* First Entry */
      tmpTable->impId = 1;
...
#define MAX_GRID 500
    ...
    ALLOC( profileData, double *, 2 );
    for( index = 0; index <= 1; index++ ) {
	ALLOC( profileData[ index ], double, 1 + numNodes );
    }
    /* the number of points is stored as profileData[0][0] */
