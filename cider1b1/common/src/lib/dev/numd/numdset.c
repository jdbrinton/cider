...
int 
NUMDsetup(matrix, inModel, ckt, states)
 ...
 */
{
  ...
  for (; model != NULL; model = model->NUMDnextModel) {
	...
	ALLOC(pDevice, 1, ONEdevice);
	ALLOC(pDevice->pStats, 1, ONEstats);
	...
	ALLOC(pDevice->elemArray, pDevice->numNodes, ONEelem *);
	...
	for (pM = materialList; pM != NIL(ONEmaterial); pM = pM->next) {
	  if (pDevice->pMaterials == NIL(ONEmaterial)) {
	    ALLOC(pMaterial, 1, ONEmaterial);
	    pDevice->pMaterials = pMaterial;
	  } else {
	    ALLOC(pMaterial->next, 1, ONEmaterial);
	...
	inst->NUMDpDevice = pDevice;
      ...
      bzero((char *) inst->NUMDpDevice->pStats, sizeof(ONEstats));
