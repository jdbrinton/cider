 ...
 */
{
  ...
  TWOmaterial *pM, *pMaterial, *materialList = NIL(TWOmaterial);
  DOPprofile *profileList = NIL(DOPprofile);
  DOPtable *dopTableList = NIL(DOPtable);
  double startTime;

  void printCoordInfo(), killCoordInfo();
  void TWOprintDomainInfo(), killDomainInfo();
  void TWOprintElectrodeInfo(), killElectrodeInfo();
  void TWObuildMesh(), TWOgetStatePointers();
  void checkElectrodes();

  /* loop through all the models */
  for (; model != NULL; model = model->NUMOSnextModel) {
    if (!model->NUMOSpInfo) {
      ALLOC(model->NUMOSpInfo, 1, TWOtranInfo);
    }
    methods = model->NUMOSmethods;
    if (!methods) {
      ALLOC(methods, 1, METHcard);
      model->NUMOSmethods = methods;
    }
    models = model->NUMOSmodels;
    if (!models) {
      ALLOC(models, 1, MODLcard);
      model->NUMOSmodels = models;
    }
    options = model->NUMOSoptions;
    if (!options) {
      ALLOC(options, 1, OPTNcard);
      model->NUMOSoptions = options;
    }
    outputs = model->NUMOSoutputs;
    if (!outputs) {
      ALLOC(outputs, 1, OUTPcard);
	...
	if (options->OPTNunique) {
	  ...
	  ALLOC(icFileName, nameLen+1, char);
	...
	ALLOC(pDevice, 1, TWOdevice);
	ALLOC(pDevice->pStats, 1, TWOstats);
	...
	ALLOC(pDevice->elemArray, pDevice->numXNodes, TWOelem **);
	for (xIndex = 1; xIndex < pDevice->numXNodes; xIndex++) {
	  ALLOC(pDevice->elemArray[xIndex], pDevice->numYNodes, TWOelem *);
	}
	...
	for (pM = materialList; pM != NIL(TWOmaterial); pM = pM->next) {
	  if (pDevice->pMaterials == NIL(TWOmaterial)) {
	    ALLOC(pMaterial, 1, TWOmaterial);
	    pDevice->pMaterials = pMaterial;
	  } else {
	    ALLOC(pMaterial->next, 1, TWOmaterial);
