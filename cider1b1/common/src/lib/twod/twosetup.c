     ...
     int index;
{
  ...
  for (nIndex = 0; nIndex <= 1; nIndex++) {
    ...
    for (eIndex = 0; eIndex <= 3; eIndex++) {
      ...
      if (pNElem ISNOT NIL(TWOelem) && pElem->elemType IS SEMICON) {
  ...
  if (card->BDRYlayerGiven && SurfaceMobility && pElem->elemType == SEMICON
    ...
    ALLOC( newChannel, TWOchannel, 1);
    newChannel->pSeed = pElem;
    newChannel->pNElem = pNElem;
    newChannel->type = index;
    if (pDevice->pChannel ISNOT NIL(TWOchannel)) {
...
void TWOsetBCparams( pDevice, cardList )
     TWOdevice *pDevice;
     BDRYcard *cardList;
{
