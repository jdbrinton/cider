...
double TWOdopingValue( pProfile, pTable, x, y )
     DOPprofile *pProfile;
     DOPtable *pTable;
     double x, y;
{
  double argX, argY, argP, argL, erfc(), lookup(), value;
  
  /* Find the appropriate lookup table if necessary */
  if (pProfile->type == LOOKUP) {
    while ( pTable ISNOT NIL(DOPtable) ) {
      if (pTable->impId IS pProfile->IMPID) {
        /* Found it */
	break;
      } else {
	pTable = pTable->next;
...
void TWOsetDoping( pDevice, pProfile, pTable )
     TWOdevice *pDevice;
     DOPprofile *pProfile;
     DOPtable *pTable;
{
