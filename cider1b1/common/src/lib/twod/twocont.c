...
TWOdevice *pDevice;
{
  ...
  int psiEqnTL, nEqnTL, pEqnTL;
  int psiEqnTR, nEqnTR, pEqnTR;
  int psiEqnBR, nEqnBR, pEqnBR;
  int psiEqnBL, nEqnBL, pEqnBL;
 ...
 */
void 
  TWO_commonTerms( pDevice, currentOnly, tranAnalysis, info )
TWOdevice *pDevice;
	...
	*(pDevice->devState0 + pEdge->edgeDpsi) = pEdge->dPsi;
	...
	if ( pElem->elemType IS SEMICON ) {
	  ...
	  bernoulli( dPsiN, &bPsiN, &dbPsiN,
		    &bMPsiN, &dbMPsiN, NOT currentOnly );
